...
$name = $phone = $Location = $price = $avaliability = "";
$name_err = $phone_err = $Location_err = $price_err = $avaliability_err = "";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $input_artist_Location = trim($_POST["Location"]);
    if (empty($input_artist_Location)) {
        $Location_err = "Please enter the Location.";
    } else {
        $Location = $input_artist_Location;
    }
    // Validate phone
    ...
    if (empty($name_err) && empty($phone_err) && empty($Location_err) && empty($price_err) && empty($avaliability_err)) {
        // Prepare an insert statement    
        $sql = "INSERT INTO halls (name, Location, PhoneNumberServiceProvider, PRICE, Availability) VALUES (?, ?, ?, ?, ?)";

        if ($stmt = mysqli_prepare($link, $sql)) {
            // Bind variables to the prepared statement as parameters
            mysqli_stmt_bind_param($stmt, "sssss", $param_name,  $param_Location, $param_phone, $param_price, $param_availability);
                                            ...
                                            <div class="row">
                                                <div class="col-md-12">
                                                    <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
                                                        ...
                                                        <div class="form-group">
                                                            <label name="Location" for="exampleFormControlSelect1">Location</label>
